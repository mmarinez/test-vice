...
class vicepage():
    ...
    def navigate_to_vice():
        Driver.driver.get("https://www.vice.com/en_us")

    def click_on_channel_videos_toggle():
        _vice_channel_ = WebDriverWait(Driver.driver, 15).until(
                                    EC.visibility_of_element_located((
                                    By.XPATH, "//div[text()='Vice Channels']")))

        _vice_channel_.click()

    def click_on_FREE_label_link():
        _FREE_label_link = WebDriverWait(Driver.driver, 15).until(
                                EC.visibility_of_element_located((
                                By.XPATH, "//img[@alt='FREE']")))

        _FREE_label_link.click()

    def validate_dek_aligment():
